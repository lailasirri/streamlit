...
def item_based_rec(picked_userid, picked_hotel, number_of_similar_items=5, number_of_recommendations=5):
    ...
    picked_userid_rating = pd.DataFrame(matrix_norm[picked_userid].dropna(axis=0, how='all').sort_values(by=picked_userid, ascending=False)).reset_index().rename(columns={picked_userid:'rating'})
    ...
    # Check if there are no recommended hotels
    if not rating_prediction:
        # If there are no recommended hotels, recommend the most popular hotels
        most_popular_hotels = agg_ratings.sort_values(by='number_of_ratings', ascending=False)['namahotel'].values.tolist()
        return most_popular_hotels[:number_of_recommendations]
    ...
    Jumlah_rekomendasi_benar = len(set(recommended_hotels).intersection(set(picked_userid_rating['namahotel'].tolist())))
    Jumlah_total_rekomendasi = number_of_recommendations
    Hitrate = Jumlah_rekomendasi_benar/Jumlah_total_rekomendasi
    # Return the recommended hotels and hit rate
    return recommended_hotels, Hitrate
